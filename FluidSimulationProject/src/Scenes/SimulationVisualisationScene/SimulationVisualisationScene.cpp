...
SimulationVisualisationScene::SimulationVisualisationScene(OpenCLContext& CLContext, RenderingSystem& renderingSystem) :
	CLContext(CLContext), renderingSystem(renderingSystem), window(renderingSystem.GetWindow()), SPHSystemRenderer(renderingSystem.GetGraphicsContext()), currentSPHSystemIndex(0)
{
	...
	SPHSystems[currentSPHSystemIndex]->Initialize(systemInitParameters);
...
void SimulationVisualisationScene::Update()
{
	...
	{
		SPHSystems[currentSPHSystemIndex]->Update(0.01f, simulationStepsPerUpdate);		
	}	

	String info;
	info += "Simulation elapsed time: " + StringParsing::Convert(SPHSystems[currentSPHSystemIndex]->GetSimulationTime()) + "s\n";
...
{
	SPHSystems[currentSPHSystemIndex]->Clear();
	currentSPHSystemIndex = std::min(index, SPHSystems.Count() - 1);
	SPHSystemRenderCache.LinkSPHSystem(SPHSystems[currentSPHSystemIndex].get());
	SPHSystemRenderCache.SetModelMatrix(Mat4f::TranslationMatrix(Vec3f(0, 0, 20)));
	uiScreen.SetImplenetationName(SPHSystems[currentSPHSystemIndex]->SystemImplementationName());
}

void SimulationVisualisationScene::LoadSystemInitParameters()
{
	File jsonFile{ "simulationProfiles/systemVisualisationProfile.json", FileAccessPermission::Read };

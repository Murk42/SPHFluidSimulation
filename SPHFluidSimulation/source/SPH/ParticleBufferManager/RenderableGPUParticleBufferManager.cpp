...
{	
	...
	RenderableGPUParticleBufferManagerWithoutCLGLInterop::RenderableGPUParticleBufferManagerWithoutCLGLInterop(cl_context clContext, cl_device_id clDevice, cl_command_queue clCommandQueue)
		...
		currentBuffer(0), bufferCL(NULL), bufferSize(0), bufferGL(0)
	{
	}
	...
	void RenderableGPUParticleBufferManagerWithoutCLGLInterop::Allocate(uintMem newBufferSize, void* ptr, uintMem bufferCount)
	{
		Clear();
	...
	uintMem RenderableGPUParticleBufferManagerWithoutCLGLInterop::GetBufferSize()
	{
		return bufferSize;
	}
	Graphics::OpenGLWrapper::GraphicsBuffer* RenderableGPUParticleBufferManagerWithoutCLGLInterop::GetGraphicsBuffer(uintMem index, uintMem& bufferOffset)
	{
		bufferOffset = 0;
		return &bufferGL;
	}
	...
	{
		return buffers[currentBuffer].LockForRendering(bufferGL, bufferSize);
	}
	void RenderableGPUParticleBufferManagerWithoutCLGLInterop::PrepareForRendering()
	{
		buffers[currentBuffer].PrepareForRendering(bufferGL, bufferSize);
	}	
	void RenderableGPUParticleBufferManagerWithoutCLGLInterop::FlushAllOperations()
	{
		
	}
	void RenderableGPUParticleBufferManagerWithoutCLGLInterop::CreateBuffers(void* ptr)
	{
		//The OpenGL buffers aren't created so they need to be created
		bufferGL = Graphics::OpenGLWrapper::ImmutableMappedGraphicsBuffer();
		bufferGL.Allocate(ptr, bufferSize, Graphics::OpenGLWrapper::GraphicsBufferMapAccessFlags::Write, Graphics::OpenGLWrapper::GraphicsBufferMapType::None);

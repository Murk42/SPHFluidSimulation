    ...
    supportedCLGLInterop(true)
{
    ...
    auto version = cl::Device(device).getInfo<CL_DEVICE_VERSION>();
    auto openCLCVersion = cl::Device(device).getInfo<CL_DEVICE_OPENCL_C_VERSION>();
    BLAZE_LOG_INFO("Successfully initialized {} ({}) with device named \"{}\"", StringView(version.data(), version.size()), StringView(openCLCVersion.data(), openCLCVersion.size()), StringView(name.data(), name.size()));

    //auto deviceExtensions = cl::Device(device).getInfo<CL_DEVICE_EXTENSIONS>();
...
void OpenCLContext::PrintPlatformAndDeviceInfo()
{
    ...
    for (auto& platform : platforms)
    {
        ...
        Console::Write("{}({})\n", StringView(name.data(), name.size()), StringView(vendor.data(), vendor.size()));
        ...
        for (auto& device : devices)
        {
            ...
            Console::Write("\t{} (type: {})\n", StringView(name.c_str(), name.size()), typeString);
        }
    }
